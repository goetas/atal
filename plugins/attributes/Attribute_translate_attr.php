...
class Attribute_translate_attr extends Attribute {
	...
	function start(xml\XMLDomElement $node, \DOMAttr $att) {
		...
		foreach ( $attrs as $attName => $attParams ){
			...
			$code .= $varName . "['$attName']=htmlspecialchars(\$__tal->getServices()->service('goetas\\\\atal\\\\plugins\\\\services\\\\ITranslate')->translate('" . addcslashes( $node->getAttribute( $attName ), "'\\" ) . "',array(" . $this->compiler->dumpKeyed( $params ) . "), $domain ),ENT_QUOTES,'UTF-8');\n";
