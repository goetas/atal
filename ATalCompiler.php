...
class ATalCompiler {
	const NS = ATal::NS;
	protected static $htmlEntities = array (
  '&uArr;' => '&#8657;',
  '&icirc;' => '&#238;',
  '&cong;' => '&#8773;',
  '&clubs;' => '&#9827;',
  ...
  '&lsquo;' => '&#8216;',
	...
	public static function splitExpression($str, $splitrer) {
		...
		for($i = 0; $i < strlen( $str ); $i ++){
			...
			if($inApex === false && $pcount == 0 && (substr( $str, $i, strlen( $splitrer ) ) == $splitrer || $i == (strlen( $str ) - 1))){
				$val = trim( substr( $str, $next, $i - $next ) );
				if(strlen( $val )){
					$parts [] = $val;
				}
				$next = $i + strlen( $splitrer );
			}
		}
		return $parts;
	}
